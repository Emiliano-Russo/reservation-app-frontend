...
export class AuthService {
    ...
    public async login(user: { username: string; password: string }) {
        const response: AxiosResponse<any> = await this.api.post('/auth/local', user);
        return response.data;
    }

    public async googleLogin(token: string) {
        const response: AxiosResponse<any> = await this.api.post('/auth/google', { token });
        return response.data;
    }

    public async googleLoginCallback() {
        const response: AxiosResponse<any> = await this.api.get('/auth/google/callback');
        return response.data;
    }

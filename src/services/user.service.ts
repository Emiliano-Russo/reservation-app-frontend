...
export class UserService {
    ...
    public registerUser(user: any) {
        ...
        return this.api.post('/users', formData, {
            headers: {
    ...
    async fetchBasicProfileInfo(userId: string): Promise<any> {
        ...
        const response: AxiosResponse<any> = await this.api.get(`/users/${userId}`, {
    ...
    async getFollowRequests(userId: string): Promise<any> {
        const jwtToken = localStorage.getItem('jwtToken');
        const response: AxiosResponse<any> = await this.api.get(`/users/follow-requests?userId=${userId}`, {
            headers: { Authorization: `Bearer ${jwtToken}` },
        });
        return response.data;
    }
    ...
    async searchByUsername(username: string): Promise<any> {
        const jwtToken = localStorage.getItem('jwtToken');
        const response: AxiosResponse<any> = await this.api.get(`/users/search/${username}`, {
            headers: { Authorization: `Bearer ${jwtToken}` },
        });
        return response.data;
    }
